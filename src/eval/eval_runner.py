"""
Eval Runner: 검색(retrieval) 평가 파이프라인.

Usage:
    python -m src.eval.eval_runner
    python -m src.eval.eval_runner --dataset eval_data/gold.json --output results/
"""
...
async def main(dataset_path: Path, output_dir: Path, k: int):
    # 데이터셋 로드 (없으면 템플릿 생성)
    ...
    runner = EvalRunner(conn, embedder, bm25, k=k)
    results = await runner.run(cases)

    # --- 리포트 출력 ---
    agg = aggregate(results)
    print("\n" + "="*60)
    print(f"Eval 결과 (K={k}, N={len(results)})")
    print("="*60)
