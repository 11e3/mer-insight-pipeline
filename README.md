...
[메르(ranto28)](https://blog.naver.com/ranto28)의 경제 블로그를 모니터링하고, Claude Batch API로 예측을 추출하며, 각 예측을 실제 결과와 대조 검증합니다 — 현재 5,020건 추적, 41,000건 뉴스 헤드라인 DB 구축 완료, 재검증 준비 중. 검색은 PostgreSQL 기반 하이브리드 BM25 + pgvector (25,090개 인덱싱된 인사이트, RRF 융합 α=0.6)로 벡터 DB 벤더 종속 없이 구현했습니다.
...
| 항목 | 수치 |
|------|------|
| 수집 소스 | Google News RSS 15개 피드 (한국어 10 + 영어 5) |
...
**Batch API 실전 결과 (1,211건):**

| 항목 | 수치 |
|------|------|
| 매칭 대상 | 5,020건 중 1,211건 (24%) |
| 검증 완료 (CORRECT+INCORRECT) | 384건 |
| CORRECT | 291건 (적중률 75.8%) |
| INCORRECT | 93건 |
| PENDING | 837건 |
| **비용** | **$0.91** (건당 $0.0024) |
| web_search 대비 | **93% 절감** ($0.035 → $0.0024) |
