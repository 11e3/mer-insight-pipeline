...
[메르(ranto28)](https://blog.naver.com/ranto28)의 경제 블로그를 모니터링하고, Claude Batch API로 예측을 추출하며, 각 예측을 실제 결과와 대조 검증합니다 — 현재 5,010건 추적 중 (664건 자동 검증 완료, 적중률 73.6%), 54,461건 뉴스 헤드라인 DB 기반 자동 검증. 검색은 PostgreSQL 기반 하이브리드 BM25 + pgvector (25,090개 인덱싱된 인사이트, RRF 융합 α=0.6)로 벡터 DB 벤더 종속 없이 구현했습니다.
...
| 검증 완료 (CORRECT+INCORRECT) | 664건 |
| CORRECT | 489건 (적중률 73.6%) |
| INCORRECT | 175건 |
| **총 비용** | **~$3** (건당 $0.0045) |
| web_search 대비 | **87% 절감** ($0.035 → $0.0045) |

**검증 불가 분류:** expected_date 없는 3,324건을 Haiku Batch API로 "뉴스 헤드라인으로 검증 가능한가?" 분류 (비용 ~$0.58):
- 검증 가능: 1,124건 → 헤드라인 매칭 + 자동 검증 대상
- 검증 불가: 2,187건 → 조건부/모호한 예측, 제외
...
**대응:** 전체 verdict 리셋 (백업 보존) → 1건씩 처리 + source_url 필수로 전환.
...
| CORRECT | 489 | 9.8% |
| INCORRECT | 175 | 3.5% |
| 검증 가능 PENDING | 1,124 | 22.4% |
| 검증 불가 (제외) | 2,187 | 43.6% |
| 미래 (expected_date 대기) | 968 | 19.3% |
| 뉴스 헤드라인 DB | 54,461 | - |
| **합계 예측** | **5,010** | |

*검증 가능 PENDING은 뉴스 DB가 매일 쌓이면서 점진적으로 자동 해결됨.*
