...
CREATE INDEX IF NOT EXISTS idx_posts_log_no ON posts (log_no);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_insights_content
    ON insights (post_id, insight_type, md5(content));
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_predictions_text
    ON predictions (md5(prediction_text));
CREATE INDEX IF NOT EXISTS idx_predictions_pending
    ON predictions (is_correct, is_verifiable)
